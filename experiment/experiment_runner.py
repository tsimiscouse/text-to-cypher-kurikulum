"""
Experiment Runner for the Agentic Text2Cypher pipeline.

Orchestrates running all 9 configurations (3 prompts × 3 schemas).
"""
import os
import json
import logging
...
class ExperimentRunner:
    """
    Runs the full experiment across all configurations.

    Manages the 3×3 factorial design (3 prompts × 3 schemas).
    """
    ...
    def run_all_configurations(
        ...
        logger.info(f"Running {len(configs_to_run)} configurations with {len(ground_truth_items)} questions each")
    ...
    def run_configuration(
        ...
        for item in ground_truth_items:
            logger.info(f"Processing question {item.id}: {item.question[:50]}...")
    ...
    def _save_configuration_results(self, config_name: str, result: ConfigurationResult):
        ...
        logger.info(f"Saved results for {config_name} to {config_dir}")
